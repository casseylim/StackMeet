...
assert.match(adminHtml, /app\.js\?v=20260723-leaderboard-loop/, "The leaderboard popup update must invalidate the browser cache.");
...
assert.match(adminClient, /const STACKMEET_APP_VERSION = "0\.9\.18"/, "Dashboard must show the current deployed app version.");
...
assert.match(adminClient, /setInterval\(\(\) => \{[\s\S]*leaderboardSlideIndex = \(leaderboardSlideIndex \+ 1\) % slides\.length/, "Leader Board display must loop through result slides.");
