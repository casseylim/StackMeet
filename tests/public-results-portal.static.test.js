...
assert.match(html, /results\.js\?v=multilanguage-v2-phase3c/, "Public Results JavaScript cache token must track the modern SignalR event model.");
...
assert.match(client, /stageDivisionAnchorId\("doubles"/, "Doubles groups must have stable anchors.");
assert.match(client, /stageDivisionAnchorId\("relay"/, "Relay groups must have stable anchors.");
assert.match(html, /id="allAroundByDivision"/, "All-Around division section must exist below aggregate standings.");
assert.match(client, /allAroundDivisionGroupsFromRows/, "Division standings must group actual published rows.");
assert.match(client, /rankAllAroundMetric/, "Division standings must rank each metric independently.");
assert.match(styles, /grid-template-columns: repeat\(4, minmax\(0, 1fr\)\)/, "Division metrics must use four desktop columns.");
assert.match(styles, /allaround-metric-grid \{ grid-template-columns: repeat\(2/, "Division metrics must collapse to two tablet columns.");
...
assert.match(client, /branding\.bannerUrl \|\| ""/);
...
assert.match(client, /publicUrlFor\(/, "Section links must use the shared public URL helper.");
assert.match(client, /const targetSection = link\.dataset\.section/, "Navigation must retain semantic result-section route values.");
assert.match(client, /encodeURIComponent\(targetSection\)/, "Result-section route values must be safely encoded without translation.");
assert.match(client, /function publicUrlFor\(path\)/, "Navigation must use the shared public URL helper.");
assert.match(client, /hasExplicitLanguage/, "Explicit public language overrides must be preserved across navigation.");
assert.match(client, /history\.replaceState/, "Public language changes must remain URL-addressable.");
assert.match(client, /backLink\.href = publicUrlFor\(resultsRoot\)/, "Dashboard/back navigation must preserve public URL behavior.");
...
assert.match(html, /results\.css\?v=public-results-quick-jump-20260818a/, "Results styles must invalidate the browser cache.");
...
assert.match(html, /results\.css\?v=public-results-quick-jump-20260818a/, "Relay styles must invalidate the browser cache.");
...
assert.match(adminClient, /if \(relayTab === "ready"\) return sorted\.filter\(relayCanCompete\);/, "Relay Ready tab must show complete teams even after competition start locks them.");
...
assert.match(adminClient, /showFinalMessage\(tf\("\{id\} saved\.[\s\S]*populateFinalSheetSelect\(\);[\s\S]*clearFinalSheet\(\);[\s\S]*sheetInput\?\.focus\(\);/, "Saving a final sheet must reset back to the Final Sheet ID finder.");
