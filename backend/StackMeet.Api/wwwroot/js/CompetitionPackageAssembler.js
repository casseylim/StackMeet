(function (root, factory) {
  ...
  async function downloadOnlinePackage({ competitionKey, competitionId, loadState, listStackers, manifest = {}, exportedAt } = {}) {
    ...
    const [state, stackers] = await Promise.all([loadState(competitionKey), listStackers(competitionId)]);
    ...
    const pkg = packageApi.createPackage(state, { ...manifest, competitionKey: String(competitionKey), competitionId }, exportedAt, { stackers });
