    ...
    AuditLogService audit) : ControllerBase
{
    ...
    public async Task<ActionResult<CertificateTemplateResponse>> Upload(int competitionId, [FromForm] string certificateType, [FromForm] string name, IFormFile file, CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(file.FileName) || originalFileName.Length == 0 || originalFileName.Length > 255) return BadRequest(new { error = "The original filename must be 255 characters or fewer." });
        if (!string.Equals(Path.GetExtension(originalFileName), ".docx", StringComparison.OrdinalIgnoreCase)
            || !string.Equals(file.ContentType, CertificateTemplateDocumentService.DocxContentType, StringComparison.OrdinalIgnoreCase))
            return BadRequest(new { error = "The upload must be a DOCX file." });
        ...
        try
        {
            ...
            var entity = new CertificateTemplate { CompetitionId = competitionId, CertificateType = type, Name = string.IsNullOrWhiteSpace(name) ? Path.GetFileNameWithoutExtension(originalFileName) : name.Trim(), OriginalFileName = originalFileName, StoredFileName = saved.StoredFileName, ContentType = CertificateTemplateDocumentService.DocxContentType, FileSize = file.Length, Sha256 = saved.Sha256, TemplateVersion = version, IsActive = false, CreatedAt = now, UpdatedAt = now, CreatedByUserId = actor, UpdatedByUserId = actor };
    ...
    async Task<ActionResult?> Access(int competitionId, CancellationToken ct, bool write = false)
    {
        ...
        if (role is null || !CompetitionPermissionService.CanManageCertificates(role)) return StatusCode(StatusCodes.Status403Forbidden);
        ...
        if (status is null || string.Equals(status, "Archived", StringComparison.OrdinalIgnoreCase) || (write && string.Equals(status, "Closed", StringComparison.OrdinalIgnoreCase))) return StatusCode(StatusCodes.Status403Forbidden);
        return null;
    }
