const storageKey = "stackmeet-stacktrack-style-v1";
...
const defaultMalayTranslations = {
  ...
  "StackTrack Settings": "Tetapan StackTrack",
...
const defaultChineseTranslations = {
  ...
  "StackTrack Settings": "StackTrack 设置",
  "Save Settings": "保存设置",
...
const demo = {
  settings: {
    name: "Seremban Junction Sport Stacking Challenge",
    type: "Sanctioned",
    start: "2026-07-05",
    end: "2026-07-05",
    prelims: "1",
...
function loadState() {
  try {
    return normalizeState(applyStartupImport(JSON.parse(localStorage.getItem(storageKey)) || structuredClone(demo)));
  } catch {
    return normalizeState(applyStartupImport(structuredClone(demo)));
  }
}

function applyStartupImport(data) {
  const imported = window.stackmeetImport4257;
  if (!imported?.stackers?.length) return data;
  const looksLikeDemo = !data.importBatch && (data.stackers?.length || 0) <= demo.stackers.length;
  if (!looksLikeDemo && data.importBatch === imported.id) return data;
  if (!looksLikeDemo) return data;

  const customDivisions = imported.stackers.map(stacker => stacker.customDivision).filter(Boolean);
  data.divisionSettings = {
    ...structuredClone(defaultDivisionSettings),
    ...(data.divisionSettings || {}),
    custom: [...new Set([...(data.divisionSettings?.custom || []), ...customDivisions])]
  };
  data.stackers = imported.stackers;
  data.doubles = [];
  data.relays = [];
  data.results = [];
  data.importBatch = imported.id;
...
function normalizeState(data) {
  ...
  data.stackers = recalculateStackerDivisions(data.stackers, data.divisionSettings, data.settings?.start);
...
function routeIsAvailable([key]) {
  ...
  if (key === "relay") return eventGroupEnabled("Timed Relay");
  return true;
}
...
function render() {
  ...
  document.getElementById("competitionType").textContent = t(`${state.settings.type} Tournament`);
  translateChrome();
...
function translateChrome() {
  ...
  document.getElementById("resetBtn").textContent = t("Reset Demo");
...
function renderDashboard() {
  ...
  document.getElementById("snapshot").innerHTML = `
    <div class="list">
      <div class="list-row"><span>Name</span><strong>${esc(state.settings.name)}</strong></div>
      <div class="list-row"><span>Date</span><strong>${esc(state.settings.start)} to ${esc(state.settings.end)}</strong></div>
...
function renderDivisionCutoffs() {
  ...
  document.getElementById("divisionCutoffs").innerHTML = groups.map(([key, label, suffix]) => `
...
function drawStackerRows() {
  ...
  document.getElementById("stackerRows").innerHTML = rows.map(s => `
    <tr>
      ...
      <td>${esc(s.gender)}</td><td><span class="pill ${s.special === "Yes" ? "blue" : ""}">${esc(s.special || "No")}</span></td><td>${esc(s.org)}</td><td>${esc(s.division)}</td><td>${esc(s.country)}</td>
      ...
      <td><button class="ghost compact-button" data-action="print-stacker-sheet" data-id="${esc(s.id)}" type="button">Print</button></td>
...
function renderRelay() {
  ...
  document.getElementById("relayRows").innerHTML = rows.map(team => {
    ...
    return `<tr>
      ...
      <td><span class="pill ${relayIsComplete(team) ? "blue" : "warning"}">${relayIsComplete(team) ? "Complete" : "Incomplete"}</span></td>
      <td>${esc(relayDivision(team))}</td>
  ...
  }).join("") || `<tr><td colspan="8"><span class="muted">No relay teams found for this tab.</span></td></tr>`;
...
function buildRelayMemberControls() {
  ...
  grid.innerHTML = Array.from({ length: 6 }, (_, index) => {
    const slot = index + 1;
    return `<label>Search Stacker ${slot}<input id="relayMemberSearch${slot}" data-relay-search="${slot}" placeholder="Name or stacker ID" /></label>
      <label>Stacker ${slot}<select id="relayMember${slot}" data-relay-member="${slot}"></select></label>`;
  }).join("");
}
...
function filteredRelaysForTab() {
  ...
  if (relayTab === "completed") return sorted.filter(relayIsComplete);
  if (relayTab === "incomplete") return sorted.filter(team => !relayIsComplete(team));
  return sorted;
}
...
function loadPrelimParticipant() {
  ...
  showPrelimMessage(`Ready for ${participant.id} ${participant.name}.`, false);
...
function resolvePrelimParticipant(rawId) {
  ...
  if (prefix === "1") {
    ...
    return stacker ? { ...config, ...stacker, name: stacker.name } : null;
  }
  if (prefix === "2") {
  ...
  return relay ? { ...relay, ...config, id: relay.id, name: participantName("Timed Relay", id), division: relayDivision(relay), org: relayLocation(relay), country: relay.country || relayCountryForMembers(relayMemberIds(relay)) } : null;
}
...
function prelimResultInputValue(result) {
  if (!result) return "";
  if (Number(result.penalty) >= 999) return "999";
  const value = official(result);
  return Number.isFinite(value) ? value.toFixed(3) : "999";
}
...
function savePrelimResults() {
  ...
  activePrelimParticipantId = participant.id;
  activePrelimParticipantType = participant.type;
  const entries = participant.events.map(event => ({ event, fieldId: prelimEventFieldIds[event], parsed: parseCompetitionTime(val(prelimEventFieldIds[event])) }));
  ...
  const idInput = document.getElementById("timeSheetId");
  idInput?.focus();
  idInput?.select();
}
...
function finalPlacements(sheet, resultRows = null) {
  const rows = (resultRows || sheet.finalists.map(finalist => finalResultFor(sheet, finalist.participant)).filter(Boolean))
    .filter(result => finalResultAttempts(result).length && Number.isFinite(finalTieBreakKey(result)[0]));
  const sorted = [...rows].sort(compareFinalResults);
  return new Map(sorted.map((result, index) => [result.participant, index + 1]));
}

function compareFinalResults(a, b) {
  const left = finalTieBreakKey(a);
  const right = finalTieBreakKey(b);
  return left[0] - right[0] || left[1] - right[1] || left[2] - right[2] || String(a.participant).localeCompare(String(b.participant), undefined, { numeric: true });
}

function finalTieBreakKey(result) {
  const times = (result?.attempts || [])
    .map(value => Number(value))
    .filter(value => Number.isFinite(value))
    .sort((a, b) => a - b);
  return [0, 1, 2].map(index => times[index] ?? Infinity);
}
...
function renderReports() {
  renderReportTabs();
  populateCompetitionReportBuilder();
...
function switchReportTab(tabName) {
  reportTab = tabName === "admin" ? "admin" : "competition";
  renderReportTabs();
}
...
function stackerDivisionFromForm() {
  ...
  return findDivisionFor(age, gender, isSpecialStacker()) || "";
}

function divisionForStacker(stacker, divisionSettings = state.divisionSettings, competitionStart = state.settings.start) {
  if (stacker.customDivision) return stacker.customDivision;
  if (stacker.standardDivision) return stacker.standardDivision;
  const age = ageOnCompetitionDate(stacker.dob, competitionStart) || Number(stacker.age);
  return findDivisionFor(age, stacker.gender, stacker.special === "Yes", divisionSettings) || "";
}

function findDivisionFor(age, gender, special = false, divisionSettings = state.divisionSettings) {
  if (!Number.isFinite(age) || age <= 0) return "";
  if (special) return findRangeName(age, divisionSettings.special || [], "Special") || "SS";
  const genderKey = gender === "F" ? "female" : "male";
  const label = gender === "F" ? "Female" : "Male";
  return findRangeName(age, divisionPath(divisionSettings, genderKey, label));
}
...
function recalculateStackerDivisions(stackers, divisionSettings = state.divisionSettings, competitionStart = state.settings.start) {
  return stackers.map(stacker => {
    const age = ageOnCompetitionDate(stacker.dob, competitionStart) || stacker.age || "";
    const updated = { ...stacker, age };
    const generatedDivision = divisionForStacker(updated, divisionSettings, competitionStart);
...
function syncStackerEditState() {
  ...
  if (cancelButton) cancelButton.hidden = !editingStackerId;
  if (printButton) {
...
function drawResultRows() {
  ...
  document.getElementById("resultRows").innerHTML = rows.map(r => `
    <tr>
      <td>${esc(r.stage)}</td><td><strong>${esc(participantName(r.type, r.participant))}</strong></td><td>${esc(r.event)}</td><td>${fmt(bestAttempt(r))}</td><td>${r.penalty >= 999 ? "Scratch" : Number(r.penalty) ? `+${r.penalty}` : "--"}</td>
  ...
  return `
    <div class="report-document" data-report-kind="admin">
      ...
      <header class="report-header">
        <p>${esc(state.settings.type)} Tournament</p>
...
function excelReportHtml(report) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    <h1>${esc(state.settings.name)}</h1><h2>${esc(report.title)}</h2>
...
document.addEventListener("click", (event) => {
  ...
  if (action === "add-stacker") addStacker();
  ...
  if (action === "confirm-delete-stacker") deleteStacker(pendingDeleteStackerId);
  ...
  if (action === "switch-relay-tab") { relayTab = target.dataset.relayTab || "completed"; shouldRender = false; renderRelay(); }
  ...
  if (action === "save-prelim-results") { savePrelimResults(); shouldRender = false; }
...
function saveSettings() {
  ...
  state.stackers = recalculateStackerDivisions(state.stackers, state.divisionSettings, state.settings.start);
}
...
async function importStackersCsvFile(event) {
  ...
  try {
    ...
    const confirmed = confirm(`Import ${stackers.length} stackers from ${file.name}? This will replace the current stacker list and clear existing teams/results.`);
    if (!confirmed) return;

    const customDivisions = stackers.map(stacker => stacker.customDivision).filter(Boolean);
    state.divisionSettings.custom = [...new Set([...(state.divisionSettings.custom || []), ...customDivisions])];
    state.divisions = generateDivisionNames(state.divisionSettings);
    state.stackers = recalculateStackerDivisions(stackers, state.divisionSettings, state.settings.start);
    state.divisions = appendStandardImportedDivisions(state.divisions, state.stackers);
    state.doubles = [];
    state.results = [];
    editingStackerId = "";
    pendingDeleteStackerId = "";
    flashMessage = { type: "success", text: `${stackers.length} stackers were imported from ${file.name}.` };
    render();
  } catch (error) {
    flashMessage = { type: "error", text: "This CSV could not be imported. Please check that it is a StackTrack Individuals report." };
    render();
  } finally {
    event.target.value = "";
  }
...
function mapStackTrackCsvRow(row) {
  ...
  stacker.division = divisionForStacker(stacker) || stacker.division || sourceDivision || "Open";
  return stacker;
}
...
function addStacker() {
  ...
  if (editingStackerId) {
    state.stackers = state.stackers.map(existing => existing.id === editingStackerId ? stacker : existing);
    flashMessage = { type: "success", text: `${stacker.name} was updated.` };
  } else {
    state.stackers.push(stacker);
    flashMessage = { type: "success", text: `${stacker.name} was added as stacker ${stacker.id}.` };
  }
  editingStackerId = "";
}
...
function deleteStacker(id) {
  ...
  state.relays = state.relays.map(team => ({ ...team, members: relayMemberIds(team).filter(member => member !== id) })).filter(team => relayMemberIds(team).length);
...
function addRelay() {
  ...
  const team = {
    ...
    customDivision: val("relayDivision").trim(),
    division: val("relayDivision").trim() || generatedRelayDivision(members),
  ...
  relayTab = relayIsComplete(team) ? "completed" : "incomplete";
...
function validateRelayEntry(members, relayName = "") {
  ...
  if (!members.length) return "Please choose at least one registered stacker.";
...
function removeConflictingRelays(stackerIds, exceptTeamId = "") {
  ...
  }).filter(team => relayMemberIds(team).length);
...
function loadRelayForEdit(id) {
  ...
  setValue("relayDivision", team.customDivision || "");
...
function clearRelayForm(renderNow = true) {
  editingRelayId = "";
  ["relayName", "relayCoordinator", "relayEmail", "relayPhone", "relayDivision", "relayRegion"].forEach(id => setValue(id, ""));
  for (let slot = 1; slot <= 6; slot += 1) {
...
function generatedRelayDivision(members) {
  const stackers = members.map(id => state.stackers.find(stacker => stacker.id === id)).filter(Boolean);
  if (!stackers.length) return "Open";
  if (stackers.some(stacker => stacker.special === "Yes")) return "SS Relay";
  const maxAge = Math.max(...stackers.map(stacker => Number(stacker.age || ageOnCompetitionDate(stacker.dob, state.settings.start) || 0)));
  if (maxAge <= 10) return "10U";
  if (maxAge <= 12) return "12U";
  if (maxAge <= 14) return "14U";
  if (maxAge <= 18) return "18U";
  return "Open";
}

function relayDivision(team) {
  return team.customDivision || team.division || generatedRelayDivision(relayMemberIds(team));
}
...
function generatedDoublesDivision(type, one, two = "") {
  ...
  if (type === "child_parent") {
    if (first?.special === "Yes") return "SS Child/Parent";
    const age = Number(first?.age || ageOnCompetitionDate(first?.dob, state.settings.start) || 0);
    return age <= 10 ? "Child/Parent 10U" : "Child/Parent 11+";
  }
  if (first?.special === "Yes" || second?.special === "Yes") {
    ...
    return maxAge <= 12 ? "SS 12U" : "SS 18U";
  }
  const maxAge = Math.max(
    Number(first?.age || ageOnCompetitionDate(first?.dob, state.settings.start) || 0),
    Number(second?.age || ageOnCompetitionDate(second?.dob, state.settings.start) || 0)
  );
  if (maxAge <= 6) return "6U";
  if (maxAge <= 8) return "8U";
  if (maxAge <= 10) return "10U";
  if (maxAge <= 12) return "12U";
  if (maxAge <= 14) return "14U";
  if (maxAge <= 18) return "18U";
  if (maxAge >= 35) return "35+";
  return "19+";
}
...
function individualTimeSheetHtml(stacker) {
  const events = ["3-3-3", "3-6-3", "Cycle"];
  const attempts = [1, 2, 3];
  return `<article class="individual-time-sheet">
    <header class="time-sheet-header">
      <div class="time-sheet-stacker-id"><span>Stacker ID</span><strong>${esc(stacker.id)}</strong></div>
      <div class="time-sheet-identity"><span>Individual</span><h2>${esc(stacker.name)}</h2></div>
      <div><span>Division</span><strong>${esc(stacker.division || "Open")}</strong></div>
      <div><span>Location</span><strong>${esc(stacker.country || "--")}</strong></div>
    </header>
    <div class="time-sheet-subline"><strong>${esc(state.settings.name)}</strong><span>${esc(stacker.org || "Independent")}</span></div>
    <table class="attempt-table">
      <thead><tr><th>Event</th>${attempts.map(attempt => `<th>Attempt ${attempt}</th>`).join("")}</tr></thead>
      <tbody>${events.map(event => `<tr><th>${event}</th>${attempts.map(() => `<td><span class="time-write-line"></span><span class="best-mark"><i></i> Judge marks best</span></td>`).join("")}</tr>`).join("")}</tbody>
    </table>
    <div class="time-sheet-notes">
      <p>Allow up to two warm-ups before Attempt 1 for each event. Record the next three stacks as the official attempts.</p>
      <p>Write the displayed time to 0.001 second. Use 999 for a scratch; leave blank if the stacker did not compete.</p>
    </div>
...
function printTimeSheetTarget(target, removableElement = null) {
  ...
  pageStyle.textContent = "@media print { @page { size: A4 portrait; margin: 10mm; } }";
...
function ageOnCompetitionDate(dobValue, competitionStart = "") {
...
function bestAttempt(result) {
  if (Number(result.penalty) >= 999) return Infinity;
  return Math.min(...result.attempts.filter(Number.isFinite));
}

function official(result) {
  const best = bestAttempt(result);
  return best === Infinity ? Infinity : best + Number(result.penalty || 0);
}
...
function stateToXml(data) {
  ...
  const divisionSettings = `<divisionSettings>\n${["combined", "male", "female", "special"].map(group => `<group name="${group}">${(data.divisionSettings?.[group] || []).map(age => node("age", age)).join("")}</group>`).join("\n")}\n<custom>${(data.divisionSettings?.custom || []).map(value => node("division", value)).join("")}</custom>\n</divisionSettings>`;
